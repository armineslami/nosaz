<div class="block md:hidden fixed bottom-0 left-0 z-10 w-full h-16 bg-background border-t border-gray-200 dark:border-gray-600">
    <div class="grid h-full max-w-lg grid-cols-4 mx-auto font-medium">
        ...
        <a href="{{ route('formula.edit') }}" type="button" class="inline-flex flex-col items-center justify-center px-5 group {{ Route::is('formula.edit') ? 'bg-primary' : 'hover:bg-primary' }}">
            <svg class="w-5 h-5 mb-2 {{ Route::is('formula.edit') ? 'text-text-900 dark:text-text-100' : 'text-gray-900 dark:text-gray-100 group-hover:text-text-900 dark:group-hover:text-text-100' }}" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" d="M9.75 3.104v5.714a2.25 2.25 0 0 1-.659 1.591L5 14.5M9.75 3.104c-.251.023-.501.05-.75.082m.75-.082a24.301 24.301 0 0 1 4.5 0m0 0v5.714c0 .597.237 1.17.659 1.591L19.8 15.3M14.25 3.104c.251.023.501.05.75.082M19.8 15.3l-1.57.393A9.065 9.065 0 0 1 12 15a9.065 9.065 0 0 0-6.23-.693L5 14.5m14.8.8 1.402 1.402c1.232 1.232.65 3.318-1.067 3.611A48.309 48.309 0 0 1 12 21c-2.773 0-5.491-.235-8.135-.687-1.718-.293-2.3-2.379-1.067-3.61L5 14.5" />
            </svg>
            <span class="text-xs {{ Route::is('formula.edit') ? 'text-text-900 dark:text-text-100' : 'text-gray-900 dark:text-gray-100 group-hover:text-text-900 dark:group-hover:text-text-100' }}">{{ __('فرمول ساز') }}</span>
        </a>
