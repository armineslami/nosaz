<div class="block md:hidden fixed bottom-0 left-0 z-10 w-full h-16 bg-background border-t border-gray-200 dark:border-gray-600">
    <div class="grid h-full max-w-lg grid-cols-4 mx-auto font-medium">
        <a href="{{ route('dashboard') }}" type="button" class="inline-flex flex-col items-center justify-center px-5 group {{ Route::is('dashboard') ? 'bg-primary border-r border-l border-r-primary-400 border-l-primary-400' : 'hover:bg-primary' }}">
        ...
        <a data-popover-target="project-menu-popover" href="{{ route('project.index') }}" type="button" class="inline-flex flex-col items-center justify-center px-5 group {{ Route::is('project.*') ? 'bg-primary border-r border-l border-r-primary-400 border-l-primary-400' : 'hover:bg-primary' }}">
            <svg class="w-5 h-5 mb-2 {{ Route::is('project.*') ? 'text-text-900 dark:text-text-100' : 'text-gray-900 dark:text-gray-100 group-hover:text-text-900 dark:group-hover:text-text-100' }}" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" d="M2.25 21h19.5m-18-18v18m10.5-18v18m6-13.5V21M6.75 6.75h.75m-.75 3h.75m-.75 3h.75m3-6h.75m-.75 3h.75m-.75 3h.75M6.75 21v-3.375c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125V21M3 3h12m-.75 4.5H21m-3.75 3.75h.008v.008h-.008v-.008Zm0 3h.008v.008h-.008v-.008Zm0 3h.008v.008h-.008v-.008Z" />
            </svg>
            <span class="text-xs {{ Route::is('project.*') ? 'text-text-900 dark:text-text-100' : 'text-gray-900 dark:text-gray-100 group-hover:text-text-900 dark:group-hover:text-text-100' }}">{{ __('پروژه‌ها') }}</span>
        </a>
        ...
        <a data-popover-target="formula-menu-popover" href="{{ route('formula.index') }}" type="button" class="inline-flex flex-col items-center justify-center px-5 group {{ Route::is('formula.*') ? 'bg-primary border-r border-l border-r-primary-400 border-l-primary-400' : 'hover:bg-primary' }}">
        ...
        <div data-popover id="formula-menu-popover" role="tooltip" class="absolute z-11 invisible inline-block w-[fit-content] transition-opacity duration-300 bg-white dark:bg-gray-700  border border-gray-200 dark:border-gray-600 rounded-lg shadow-sm opacity-0">
            <ul class="mb-2">
                ...
                <li>
                    <a href="{{ route('formula.label.create') }}" class="flex text-sm w-full px-4 py-2 {{ Route::is('formula.variable.create') ? 'bg-primary text-text-900 dark:text-text-100 border-b border-b-primary-400': 'text-text hover:text-text-900 hover:dark:text-text-100' }} transition duration-75 pl-11 group hover:bg-primary">
                        {{ __('برچسب‌ها') }}
                    </a>
                </li>
            </ul>
        ...
        <a href="{{ route('reports') }}" type="button" class="inline-flex flex-col items-center justify-center px-5 group {{ Route::is('reports') ? 'bg-primary border-r border-l border-r-primary-400 border-l-primary-400' : 'hover:bg-primary' }}">
