<x-app-layout>
    <div class="max-w-7xl mx-auto">
        ...
        <x-card class="mt-8">
            <form method="POST" action="{{ route('formula.variable.store') }}">
                ...
                <x-text-input id="name" class="block my-4 w-full"
                    placeholder="{{ __('مانند: درصد مشارکت، هزینه ساخت، ...') }}" type="text" name="name"
        ...
        @if (session('status') === 'variable-created')
            ...
            setTimeout(() => {
                ...
                message="{{ __('متغیر با موفقیت ایجاد شد') }}" />
        @elseif (session('status') === 'variable-not-created')
            <x-toast x-data="{ show: true }" x-show="show" x-init="$el.classList.add('toast-transition-in');
            $el.classList.remove('hidden');
            setTimeout(() => {
                $el.classList.remove('toast-transition-in');
