...
{{--                <textarea id="formulaBuilderTextArea" name="formula" class="hidden"></textarea>--}}
            ...
            @else
                ...
                <div id="variablesArea" class="flex flex-wrap gap-2 mt-2 md:mt-4">
                    @foreach($variables as $variable)
                        <div data-name="{{ $variable->name }}" data-id="{{ $variable->id }}" class="variables clickableVariable secondary-button shadow-md bg-primary py-2 px-3 min-w-12 rounded-md text-text text-sm cursor-pointer">
        ...
        @if (session('status') === 'formula-not-deleted')
            <x-toast
                ...
                x-transition
                x-init="
                        $el.classList.add('toast-transition-in');
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const hiddenFormulaPayload = document.getElementById("hiddenFormulaPayload");
            ...
            function restoreFormula(formula) {
                ...
                for(let i = 0; i < formula.length; i++) {
                    let value = formula[i];
                    if (value === '<') {
                        insertStorageMode = true;
                    }
                    else if (value === '>') {
                        insertStorageMode = false;
                        if (newLineMode) {
                            insertNewLine();
                            newLineMode = false;
                        }
                    ...
                    else if (insertStorageMode) {
                        const data = sliceAndMoveIndex(formula, i, '<', '>');
                        insertVariable(formulaBuilder, data.slicedPart, null, true, false);
            ...
            function sliceAndMoveIndex(str, i, startChar, endChar) {
                ...
                if (startPos !== -1 && endPos !== -1) {
                    ...
                    return { slicedPart, updatedIndex: i };
                }

                // If no matching endChar is found, return null and leave index unchanged
                return { slicedPart: null, updatedIndex: i };
            }
            ...
            function submitFormulaForm(formula) {
                const formulaIdElement = document.getElementById('formulaId');
                ...
                axios.patch(`/formula/${formulaIdElement.innerText}`, {name:formulaNameElement.value, formula})
                    .then(response => {
