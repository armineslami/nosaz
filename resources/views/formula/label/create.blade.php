<x-app-layout>
    <div>
        ...
        <x-card>
            <form method="POST" action="{{ route('formula.label.store') }}">
                @csrf
        ...
        <div class="mt-6">
            ...
            <x-card id="labelsCard" class="min-h-28 {{ empty($labels) ? 'flex justify-center' : ''}}">
                ...
                @if(!empty($labels))
                    <div id="labelsContainer"  class="flex gap-2">
            ...
            deleteButtons.forEach(function(button) {
                button.addEventListener('click', function() {
                    ...
                    if (spinner.classList.contains('hidden')) {
                        deleteLabel(labelId, parent, this, spinner);
                    }
                });
            });

            function deleteLabel(id, parentElement, deleteButton, spinner) {
                deleteButton.classList.add('hidden');
                spinner.classList.remove('hidden');

                axios.delete(`/formula/label/${id}`)
                    .then(response => {
